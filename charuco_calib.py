...
SQUARE_LENGTH = 0.04  # Adjust based on your printed board (in meters)
MARKER_LENGTH = 0.02  # Adjust based on your printed board (in meters)
...
def capture_images(video_source="COM_PORT", num_images=20):
    ...
    while count < num_images:
        ...
        corners, ids, _ = aruco.detectMarkers(frame, CHARUCO_DICT)
        frame = aruco.drawDetectedMarkers(frame, corners, ids)
        cv2.imshow("Frame", frame)
        
        key = cv2.waitKey(1) & 0xFF
        if key == ord('s') and ids is not None:  # Save image on 's' press
            count += 1
            process_image(frame)
...
def process_image(image):
    """ Process an image to detect ChArUco corners. """
    global image_size
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    corners, ids, _ = aruco.detectMarkers(gray, CHARUCO_DICT)
    if ids is not None:
        _, charuco_corners, charuco_ids = aruco.interpolateCornersCharuco(corners, ids, gray, CHARUCO_BOARD)
...
def calibrate_camera():
    """ Perform camera calibration using collected ChArUco data. """
